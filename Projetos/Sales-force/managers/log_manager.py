...
from extras.maps import REORDER_COLUMNS
from managers.config import config as c
from managers.file_manager import reorder_dict, to_str, get_sum
...
def log_exception(e: Exception, msg: str, card: dict=None, exit: bool=False):
    ...
    save_error(dt_error_date, filename, str(func_name), error_line, str(e), str(detailed_exp), msg)
...
def save_error(dt_error_date: dt, filename: str, func_name: str, line:int , exec: str, detailed_exp: str, e_msg: str):
    ...
    try:
        ...
        add_log('| ERROR LOG | Gerando log de erro pro Oracle', type='e')
        send_card({
            'TITLE': 'GERANDO LOG DE ERRO',
            'TITLE_SUBTITLE': 'Montando dict final e query',
            'MSG': 'Falha ao gerar dicionário de log de erro e query pro banco'})
    ...
    try:
        ...
        add_log('| ERROR LOG | Salvando log de erro no Oracle', type='e')
        send_card({
            'TITLE': 'ENVIANDO LOG DE ERRO',
            'TITLE_SUBTITLE': 'Enviando dict final e query',
            'MSG': 'Falha ao enviar dicionário de log de erro e query pro banco'})
...
def close_log(total_time: float, all_json_logs: list, extraction_date: str):
    ...
    try:
        ...
        end_date = get_date(date_form=True)

        final_dict = {
            'ext': get_time(ext_sum), 
            'tra': get_time(tra_sum), 
            'loa': get_time(loa_sum), 
            'err': qtt_error,
            'tot': get_time(total_time),
            'tab': len(all_json_logs)}
        ...
        log_exception(e, '| TABELA LOG | Gerando log geral', {
            'TITLE': 'GERANDO LOG DE TABELA',
            'TITLE_SUBTITLE': 'Montando dict final',
            'MSG': 'Falha ao gerar dicionário com dados da tabela'})
        

    # ---- Add colunas EXTRAS ----
    try:
        final_list = [{**json, 'DS_TEMPO_TOTAL': None, 'QT_TEMPO_TOTAL_SEG': None, 'QT_ERROS': None, 'DH_FIM_EXECUCAO': None} for json in all_json_logs]
        final_list = reorder_dict(final_list, REORDER_COLUMNS['LOG'])
        final_list.append(full_routine)   
    
    except Exception as e:
        log_exception(e, '| TABELA LOG | Adicionando colunas extras na geração do log geral', {
            'TITLE': 'GERANDO LOG DE TABELA',
            'TITLE_SUBTITLE': 'Adicionando colunas extras',
            'MSG': 'Falha ao adicionar colunas extras no log das tabelas'})
        

    # ---- SAVE log ----
    try:
        columns = REORDER_COLUMNS['LOG']
        columns_enum = [':' + str(i+1) for i in range(len(columns))]
        ...
        log_exception(e, '| TABELA LOG | Salvando logs de tabela na CRTL do Oracle', {
            'TITLE': 'SALVANDO TABELA LOG',
            'TITLE_SUBTITLE': 'Salvando no CTRL no Oracle',
            'MSG': 'Falhando enquanto enviada a tabela para o LOG'})
    ...
    send_card({
        ...
        'MSG': None}, error=qtt_error > 0, final=final_dict)
...
def call_procedure(action: str='END', error_log: str=None, row_count: int=0):
    ...
    try:
        params = {
            'p_package': c.r_procedure,
            'p_procedure': c.r_procedure,
            'p_command': 'CARGA',
            'p_action': action,
        ...
        with open_oracle_conn() as conn:
            cursor = conn.cursor()
            cursor.callproc(
                'BI_DW.PC_DW_UTIL_CONTROLE.SP_LOG_COMM_NOVO',
                [
                    params['p_package'],
                    params['p_procedure'],
                    params['p_command'],
        ...
        log_exception(e, '| PACKAGE | Chamando a package', {
            'TITLE': 'CHAMANDO PACKAGE',
            'TITLE_SUBTITLE': f'Fazendo a chamada: {action}',
            'MSG': 'Falhando na chamada da Package'})
